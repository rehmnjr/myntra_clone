...
products.forEach(i =>{
    let element = 'li';
    let list = document.createElement(`${element}`);
    ...
    <div class="detail">
    <h3>${i["Brand Name"]}</h3>
    <span class="product-name"> ${i["Color"]} ${i["Product Name"]} for ${i["Gender"]}</span>
    <span class="price">Rs. ${i["Price"]} <strike class="strike-price ">Rs. ${ Math.round(((i["Price"]*100)/i["Discount"])*100)/100}</strike> <span class="per-cent">(${i["Discount"]}% OFF)</span> </span></span>
    </div> `
